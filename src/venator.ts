...
// only one of the coordinates in `from` have to pass
// *all* of the coordinates in `to` have to be in range of a coordinate in `from`
...
export function canBounce(
  ...
  const [, , center] = getScanTiles(x, y, size);
  ...
  switch (size) {
    ...
    case 4:
      ...
      const middle2x2 = getAllTiles(x + 1, y - 1, 2);
